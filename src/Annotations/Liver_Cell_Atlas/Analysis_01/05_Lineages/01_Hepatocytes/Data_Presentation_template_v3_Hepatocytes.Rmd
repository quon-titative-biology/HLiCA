...
for (l in levels(top10$cluster)){
  if (nrow(top10[top10$cluster==l,]) > 0) { # Only print genes if there are marker genes for that cluster
    ...
    cat("\n")
    cat("####","Violin","\n")
    p <- VlnPlot(seurat, features=genes,ncol=2, pt.size = 0, cols = opts$discrete_col_pal) & geom_jitter(height = 0, size = 0.5, show.legend = FALSE, alpha=0.1)
    ...
    cat("\n")
    cat("####","Split Violin","\n")
    if (nlevels(factor(seu$condition)) > 1){
      p <- VlnPlot(seurat, features=genes,ncol=1, split.by = "condition", pt.size = 0, cols = opts$discrete_col_pal) & theme(legend.position = "right")
