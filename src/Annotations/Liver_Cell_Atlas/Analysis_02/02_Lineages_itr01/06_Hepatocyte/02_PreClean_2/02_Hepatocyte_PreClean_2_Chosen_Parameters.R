# Liver Cell Atlas - Hepatocyte (PreClean 2) - Chosen Parameters
...
obj <- readRDS("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/01_PreClean/chosen_parameters/Hepatocyte_unclean.rds")
seu <- obj$seu
opts <- list()
...
##-----------------------------------------------------------------------
opts$lineage_anno <- "Hepatocyte" #subset lineage
...
path <- "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/02_PreClean_2/"
...
sink_name <- "02_Hepatocyte_PreClean_2_chosen_parameters_console" # console_output
...
opts$Hepatocyte_unclean_anno_dir_2 <- io$out_dir
saveRDS(object = list(seu = seu, opts = opts),
        file = paste0(opts$Hepatocyte_unclean_anno_dir_2,"/", "Hepatocyte_unclean_2.rds"))
...
# For simplicity in the other lineages I just scaled all genes for this analysis (these scaled matrix isnt saved)
# For the hepatocytes, due to memory issues and time, I only scale those genes required, its does not affect results as far as I can see.
...
tmp <- seu
tmp <- ScaleData(tmp, features = c(unlist(opts$modules_group$Atlas_general),unlist(opts$modules_group$Atlas_specific),unlist(opts$modules_group$Hendo)))
...
dim_plot(seu, group.by = "HCA_General_sctype", raster = FALSE)
dim_plot(seu, group.by = "HCA_Specific_sctype", raster = FALSE)
dim_plot(seu, group.by = "Hendo_sctype", raster = FALSE)
...
DimPlot(seu, group.by = "malat1_threshold", raster=FALSE)
...
# Cell Annotations
# Annotating the doublets in this round of cleaning

seu$Beta.Annotation.Lineage <- "Hepatocyte"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "13"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "14"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "17"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "20"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "26"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "33"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "34"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "35"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "36"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "37"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "38"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "39"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "40"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "41"] <- "Doublet"
seu$Beta.Annotation.Lineage[seu$seurat_clusters == "42"] <- "Doublet"
table(seu$Beta.Annotation.Lineage)



seu$Potential.Doublets <- "FALSE"
seu$Potential.Doublets[seu$seurat_clusters == "13"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "14"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "17"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "20"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "26"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "33"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "34"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "35"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "36"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "37"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "38"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "39"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "40"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "41"] <- "TRUE"
seu$Potential.Doublets[seu$seurat_clusters == "42"] <- "TRUE"

seu$Potential.Doublets[seu$seurat_clusters == "16"] <- "TRUE"

table(seu$Potential.Doublets)


DimPlot(seu, group.by = "Beta.Annotation.Lineage", raster=FALSE)
DimPlot(seu, group.by = "Potential.Doublets", raster=FALSE)

df <- seu@meta.data[,c("Beta.Annotation.Lineage","Potential.Doublets")]
write.csv(df, "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/02_PreClean_2/chosen_parameters/Hepatocyte_unclean_annotations_2.csv")
...
presentation_create(title = "Liver Cell Atlas - Hepatocyte (Unclean 2) - Data Presentation", 
                    template = "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/02_PreClean_2/Hepatocyte_PreClean_2_template.Rmd", 
                    obj_path = "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/02_PreClean_2/chosen_parameters/Hepatocyte_unclean_2.rds", 
                    results_path = "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/02_PreClean_2/chosen_parameters/",
                    final_cluster_param = "Beta.Annotation.Lineage",
                    output_file = "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/02_PreClean_2/Hepatocyte_Unclean_2_Data_Presenation.html")


# Intermediate Final Annotation File

hepatocyte_df <- read.csv("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/01_PreClean/chosen_parameters/Hepatocyte_unclean_annotations.csv", row.names = 1)
hepatocyte_df[names(seu$Beta.Annotation.Lineage),"Beta.Annotation.Lineage"] <- as.character(seu$Beta.Annotation.Lineage)

hepatocyte_df[names(seu$Potential.Doublets),"Potential.Doublets"] <- as.character(seu$Potential.Doublets)


# Test that the above re naming is correct
test <- seu
test$Beta.Annotation.Lineage <- NULL
test$Potential.Doublets <- NULL
test <- AddMetaData(test, hepatocyte_df)

all.equal(as.character(test$Beta.Annotation.Lineage), as.character(seu$Beta.Annotation.Lineage))
all.equal(as.character(test$Potential.Doublets), as.character(seu$Potential.Doublets))
DimPlot(test, group.by = "Beta.Annotation.Lineage", raster = FALSE)
DimPlot(test, group.by = "Potential.Doublets", raster = FALSE)

write.csv(hepatocyte_df, "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/02_PreClean_2/chosen_parameters/Hepatocyte_Intermediate_annotations.csv")




### Save Harmony Components
obj <- readRDS("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/06_Hepatocyte/02_PreClean_2/chosen_parameters/Hepatocyte_unclean_2.rds")
seu <- obj$seu
opts <- obj$opts
path <- paste0(opts$Hepatocyte_unclean_anno_dir_2, "/harmony_backup/")
