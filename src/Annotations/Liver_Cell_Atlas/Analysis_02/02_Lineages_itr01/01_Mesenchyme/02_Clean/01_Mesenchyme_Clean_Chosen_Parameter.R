# Liver Cell Atlas - Mesenchyme - Chosen Parameters
...
obj <- readRDS("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/01_PreClean/chosen_parameters/Mesenchyme_unclean.rds")
seu <- obj$seu
opts <- list()
...
##-----------------------------------------------------------------------
opts$lineage_anno <- "Mesenchyme" #subset lineage
...
path <- "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/02_Clean/"
...
sink_name <- "01_Mesenchyme_Clean_chosen_parameters_console" # console_output
...
opts$res <- seq(from = 0.1, to = 0.1, by = 0.1)
# Batch ID to perform integration
opts$batch_id <- covariates
# QC information to plot
opts$qc_to_plot <- c("nCount_RNA","nFeature_RNA", "percent.mt","MALAT1")
...
opts$discrete_col_pal <- c("#D26263", "#6399CA", "#FABD6A", "#CBB3D8", "#FD9D9C", "#CD8240", "#6D8938", "#A7A6A7", "#7CBEEB", "#A883FD", "#9ACB9A", "#EEE484", 
                           ...
                           "#00FFB2", "#FB00CB", "#890076", "#FB9CD4", "#BA001C", "#A500FA", "#A36375", "#FE0DFA")
...
annotations <- list(Beta.Annotation.Lineage = c("0" = "Mesenchyme",
                                                "1" = "Mesenchyme",
                                                "2" = "Mesenchyme",
                                                "3" = "Mesenchyme",
                                                "4" = "Neuronal",
                                                "5" = "Doublet",
                                                "6" = "Mesothelia"),
                    Beta.Annotation.SubLineage = c("0" = "Vascular Smooth Muscle Cell",
...
write.csv(df, "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/02_Clean/chosen_parameters/Mesenchyme_Clean_annotations.csv")

mesenchyme_df <- read.csv("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/01_PreClean/chosen_parameters/Mesenchyme_unclean_annotations.csv", row.names = 1)
mesenchyme_df[names(seu$Beta.Annotation.Lineage),"Beta.Annotation.Lineage"] <- as.character(seu$Beta.Annotation.Lineage)
mesenchyme_df$Beta.Annotation.SubLineage <- "TBC"
mesenchyme_df[names(seu$Beta.Annotation.SubLineage),"Beta.Annotation.SubLineage"] <- as.character(seu$Beta.Annotation.SubLineage)
mesenchyme_df$Potential.Doublets.2 <- "FALSE"
mesenchyme_df[names(seu$Potential.Doublets.2),"Potential.Doublets.2"] <- as.character(seu$Potential.Doublets.2)
...
test <- AddMetaData(test, mesenchyme_df)
...
write.csv(mesenchyme_df, "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/Mesenchyme_annotations.csv")

# Save Data
# Save updated metadata
opts$Mesenchyme_clean_anno_dir <- io$out_dir
saveRDS(object = list(seu = seu, opts = opts),
        file = paste0(opts$Mesenchyme_clean_anno_dir,"/", "Mesenchyme_clean.rds"))
...
presentation_create(title = "Liver Cell Atlas - Mesenchyme (Clean ish) - Data Presentation", 
                    template = "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/02_Clean/Mesenchyme_Clean_template.Rmd", 
                    obj_path = "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/02_Clean/chosen_parameters/Mesenchyme_clean.rds", 
                    results_path = "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/02_Clean/chosen_parameters/",
                    final_cluster_param = c("Beta.Annotation.Lineage","Beta.Annotation.SubLineage", "Potential.Doublets.2"),
                    output_file = "~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/02_Clean/Mesenchyme_Clean_Data_Presenation.html")


### Save Harmony Components
obj <- readRDS("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/01_Mesenchyme/02_Clean/chosen_parameters/Mesenchyme_clean.rds")
seu <- obj$seu
opts <- obj$opts
path <- paste0(opts$Mesenchyme_clean_anno_dir, "/harmony_backup/")
