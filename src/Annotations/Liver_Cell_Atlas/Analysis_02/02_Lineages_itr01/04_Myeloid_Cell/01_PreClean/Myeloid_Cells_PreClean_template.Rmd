...
tmp <- seurat
tmp$seurat_clusters <- factor(seurat$seurat_clusters, levels = c(0,1,2,3,4,7,8,10,11,12,13,16,5, 6, 9, 14,15))
Idents(tmp) <- "Alpha.Annotations.tmp"

plot <- DotPlot(tmp, features = unique(c(unlist(opts$modules_group$Hendo), "NRXN1","ERBB3","SOX10","S100B")), group.by = "seurat_clusters") + ggplot2::coord_flip(x=c(1,73), y=c(1,17),clip = "off") + 
  ...
  geom_brace(xstart = -1, xend = -3, ystart = 1, yend = 12,pointing="side") +
  annotate("text",x=-5,y=6,label="Myeloid Cell",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 13, yend = 13,pointing="side") +
  annotate("text",x=-4,y=13,label="Lymphocyte",angle = 45) +
  ...
  annotate("text",x=-4,y=15,label="Hepatocyte",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 16, yend = 17,pointing="side") +
  annotate("text",x=-4,y=16.5,label="Doublet",angle = 45) +
