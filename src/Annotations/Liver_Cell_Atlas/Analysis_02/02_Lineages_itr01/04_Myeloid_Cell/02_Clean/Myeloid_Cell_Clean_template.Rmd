...
# Map Clusters to Unclean Myeloid Cell Dataset  {.tabset}

## Plots

```{r Map Data Back - Unclean MP, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
unclean_obj <- readRDS("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/04_Myeloid_Cell/01_PreClean/chosen_parameters/Myeloid_Cells_unclean.rds")
...
subchonkify(wrap_plots(plots, ncol = 3), fig_width=15, fig_height=5*(ceiling((length(plots)/3))), chunk_name = "MapBack_MP", loop_variable = 1, random_id = 1)
```

## Table

```{r Map Clusters table - Unclean MP, echo=FALSE}
seurat_meta <- seurat@meta.data[,"seurat_clusters", drop=FALSE ]
colnames(seurat_meta) <- "Lineage_Clustering"
...
# Myeloid Cell Sub Population Genes - Henderson List {.tabset}



```{r MP Sub Pop Markers, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
DefaultAssay(seurat) <- "RNA"
sub_pop <- list(Monocytes_General = c("CD300E"),
                Monocyte_Classical = c("S100A12"),
                Monocyte_Non_Classical = c("NEURL1"),
               Macrophages_General = c("AXL", "MERTK", "C1QC"),
              Kupffer = c("CD5L", "TIMD4", "MARCO"),
              cDCs_Type1 = c("XCR1", "CLEC9A"),
              cDCs_Type2 = c("CD1C", "CD1E", "FCER1A", "CLEC10A"))
