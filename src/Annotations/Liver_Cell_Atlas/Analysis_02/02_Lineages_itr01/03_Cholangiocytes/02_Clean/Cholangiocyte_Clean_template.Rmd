...
# Map Clusters to Unclean Cholangiocytes Dataset  {.tabset}

## Plots

```{r Map Data Back - Unclean Cho, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
unclean_obj <- readRDS("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/03_Cholangiocytes/01_PreClean/chosen_parameters/Cholangiocyte_unclean.rds")
...
subchonkify(wrap_plots(plots, ncol = 3), fig_width=15, fig_height=5*(ceiling((length(plots)/3))), chunk_name = "MapBack_Cho", loop_variable = 1, random_id = 1)
```

## Table

```{r Map Clusters table - Unclean Cho, echo=FALSE}
seurat_meta <- seurat@meta.data[,"seurat_clusters", drop=FALSE ]
colnames(seurat_meta) <- "Lineage_Clustering"
...
# Cholangiocytes Sub Population Genes - Henderson List {.tabset}



```{r Cholangiocyte Sub Pop Markers, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
DefaultAssay(seurat) <- "RNA" 
sub_pop <- list(Large = c("MUC5B",	"FCGBP"),
                      Small = c("DSCAM",	"SEMA6D"))
