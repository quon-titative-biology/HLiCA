...
tmp <- seurat
tmp$seurat_clusters <- factor(seurat$seurat_clusters, levels = c(0,1,3,5,9,10,13,14,15, 2, 4, 6, 7, 8, 16, 17, 11,12 ))
Idents(tmp) <- "Alpha.Annotations.tmp"

plot <- DotPlot(tmp, features = unique(c(unlist(opts$modules_group$Hendo), "NRXN1","ERBB3","SOX10","S100B")), group.by = "seurat_clusters") + ggplot2::coord_flip(x=c(1,73), y=c(1,18),clip = "off") + 
  ...
  geom_brace(xstart = -1, xend = -3, ystart = 1, yend = 9,pointing="side") +
  annotate("text",x=-5,y=4.5,label="Cholangiocyte",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 10, yend = 11,pointing="side") +
  annotate("text",x=-4,y=11,label="Hepatocyte",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 12, yend = 12,pointing="side") +
  annotate("text",x=-4,y=12,label="Endothelia",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 13, yend = 13,pointing="side") +
  annotate("text",x=-4,y=13,label="Myeloid Cell",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 14, yend = 14,pointing="side") +
  annotate("text",x=-4,y=14,label="Lymphocyte",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 15, yend = 15,pointing="side") +
  annotate("text",x=-4,y=15,label="Mesenchyme",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 16, yend = 16,pointing="side") +
  annotate("text",x=-4,y=16,label="Erythrocyte",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 17, yend = 17,pointing="side") +
  annotate("text",x=-4,y=17,label="Neuronal",angle = 45) +
  geom_brace(xstart = -1, xend = -3, ystart = 18, yend = 18,pointing="side") +
  annotate("text",x=-4,y=18,label="Doublet",angle = 45) +

  geom_brace(xstart = 1, xend = 7, ystart = -1.5, yend = -3) +
  annotate("text",x=3,y=-4,label="Hepatocytes",angle = 45) +
  geom_brace(xstart = 8, xend = 14, ystart = -1.5, yend = -3) +
  annotate("text",x=10.5,y=-4,label="Cholangiocytes",angle = 45) +
  geom_brace(xstart = 15, xend = 22, ystart = -1.5, yend = -3) +
  annotate("text",x=17.5,y=-4,label="Endothelia",angle = 45) +
  geom_brace(xstart = 23, xend = 31, ystart = -1.5, yend = -3) +
  annotate("text",x=26,y=-4,label="Mesenchyme",angle = 45) +
  geom_brace(xstart = 32, xend = 33, ystart = -1.5, yend = -3) +
  annotate("text",x=31.5,y=-4,label="Mesothelia",angle = 45) +
  geom_brace(xstart = 34, xend = 37, ystart = -1.5, yend = -3) +
  annotate("text",x=34.5,y=-4,label="Leucocytes",angle = 45) +
  geom_brace(xstart = 38, xend = 43, ystart = -1.5, yend = -3) +
  annotate("text",x=40.5,y=-3.5,label="MPs",angle = 45) +
  geom_brace(xstart = 44, xend = 46, ystart = -1.5, yend = -3) +
  annotate("text",x=44,y=-4,label="Lymphocytes",angle = 45) +
  geom_brace(xstart = 47, xend = 52, ystart = -1.5, yend = -3) +
  annotate("text",x=49,y=-3.7,label="T Cells",angle = 45) +
  geom_brace(xstart = 53, xend = 54, ystart = -1.5, yend = -3) +
  annotate("text",x=53,y=-3.7,label="B Cells",angle = 45) +
  geom_brace(xstart = 55, xend = 56, ystart = -1.5, yend = -3) +
  annotate("text",x=55.5,y=-3.5,label="ILCs",angle = 45) +
  geom_brace(xstart = 57, xend = 58, ystart = -1.5, yend = -3) +
  annotate("text",x=57,y=-3.8,label="Mast Cells",angle = 45) +
  geom_brace(xstart = 59, xend = 60, ystart = -1.5, yend = -3) +
  annotate("text",x=59.5,y=-3.5,label="PDCs",angle = 45) +
  geom_brace(xstart = 61, xend = 64, ystart = -1.5, yend = -3) +
  annotate("text",x=61.5,y=-3.8,label="Erythrocytes",angle = 45) +
  geom_brace(xstart = 65, xend = 65, ystart = -1.5, yend = -3) +
  annotate("text",x=64,y=-3.8,label="Neutrophils",angle = 45) +
  geom_brace(xstart = 66, xend = 69, ystart = -1.5, yend = -3) +
  annotate("text",x=67,y=-3.8,label="Cycling",angle = 45) +
  geom_brace(xstart = 70, xend = 73, ystart = -1.5, yend = -3) +
  annotate("text",x=71.5,y=-3.8,label="Neuronal",angle = 45) +
...
```{r Cluster Highlight, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
plots <- list()
for (i in levels(seurat$seurat_clusters)){
  plot1 <- DimPlot(seurat, cells.highlight = colnames(seurat)[seurat$seurat_clusters == i], raster = FALSE) & ggtitle(paste0("Cluster ",i))
  plots[[i]] <- plot1
}
subchonkify(wrap_plots(plots, ncol = 3), fig_width=15, fig_height=5*(ceiling((length(plots)/3))), chunk_name = "ClusterHighlight", loop_variable = 1, random_id = 1)
```
