...
Percentage of the Cells that are either True or False for the Malat Threshold per cluster. False denotes they did not meet the threshold (i.e. they are lower than it).

```{r Malat Thresold, echo=FALSE}
malat_thres_percent <- prop.table(table(seurat$malat1_threshold, seurat$seurat_clusters), margin = 2)*100

t(malat_thres_percent) %>%
  kbl() %>%
  kable_styling()
```
...
# Map Clusters to Unclean Lymphocyte Dataset  {.tabset}

## Plots

```{r Map Data Back - Unclean Lym, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
unclean_obj <- readRDS("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/05_Lymphocyte/01_PreClean/chosen_parameters/Lymphocyte_unclean.rds")
...
subchonkify(wrap_plots(plots, ncol = 3), fig_width=15, fig_height=5*(ceiling((length(plots)/3))), chunk_name = "MapBack_Lym", loop_variable = 1, random_id = 1)
```

## Table

```{r Map Clusters table - Unclean Lym, echo=FALSE}
seurat_meta <- seurat@meta.data[,"seurat_clusters", drop=FALSE ]
colnames(seurat_meta) <- "Lineage_Clustering"
...
# Lymphocyte Sub Population Genes - Henderson List {.tabset}



```{r MP Sub Pop Markers, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
DefaultAssay(seurat) <- "RNA" 
sub_pop <- list(T_Cells_Helper_Genereal = c("CD4", "IL7R"),
                T_Cell_Regulatory = c("IL2RA", "FOXP3", "CTLA4"),
                T_Cell_Cytotoxic = c("CD8A", "CD8B"),
                T_Cell_MAIT = c("SLC4A10"),
                NK_Cells_General = c("KLRF1", "KLRC1", "FCER1G", "NCAM1", "TRDC"),
                NK_Cell_Bright = c("FCGR3A", "PTGDS", "IGFBP7"),
                NK_Cell_Dim = c("TOX2"),
                ILC_Type2 = c("KIT", "IL1R1"),
                B_Cells_Naive = c("CD19", "MS4A1", "BANK1"),
                B_Cell_Plasma = c("JCHAIN", "IGHA2", "IGHG1", "IGHG2", "IGHGP", "IGLL5"))
