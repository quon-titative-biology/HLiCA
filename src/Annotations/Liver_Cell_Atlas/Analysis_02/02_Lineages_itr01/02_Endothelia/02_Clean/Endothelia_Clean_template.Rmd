...
# Map Clusters to Unclean Endothelia Dataset  {.tabset}

## Plots

```{r Map Data Back - Unclean End, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
unclean_obj <- readRDS("~/project/Liver_Cell_Atlas/Analysis_02/02_Lineages_itr01/02_Endothelia/01_PreClean/chosen_parameters/Endothelia_unclean.rds")
...
subchonkify(wrap_plots(plots, ncol = 3), fig_width=15, fig_height=5*(ceiling((length(plots)/3))), chunk_name = "MapBack_End", loop_variable = 1, random_id = 1)
```

## Table

```{r Map Clusters table - Unclean End, echo=FALSE}
seurat_meta <- seurat@meta.data[,"seurat_clusters", drop=FALSE ]
colnames(seurat_meta) <- "Lineage_Clustering"
...
# Endothelia Sub Population Genes - Henderson List {.tabset}



```{r Endothelia Sub Pop Markers, results='asis', message=FALSE, warning=FALSE, echo=FALSE}
DefaultAssay(seurat) <- "RNA" 
sub_pop <- list(Macrovascular = c("VWF", "ANO2"),
                  Hepatic_Artery = c("SYT1", "FBLN5", "SEMA3G"),
                  Portal_Vein =  c("CPE", "CDH11", "CD320"),
                  Fibrotic_Injury = c("PLVAP", "ACKR1", "VWA1"),# https://www.nature.com/articles/s41586-019-1631-3
                  Central_Vein = c("RSPO3"),
                  LSECs = c("CLEC4G", "CLEC4M", "CD14"),
                  Lymphatic = c("PROX1", "PDPN"))
