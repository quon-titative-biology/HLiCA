...
load(here("scratch/HLiCA/seu_cleaned_endo.RData"))
...
save_DEG_plots<-function(celltype_num){
  ...
  if(nrow(plt_path)==0){}else{
    ...
    endo_pathway<-ggplot(plt_path, aes(NES, reorder(pathway, NES)))+geom_point(aes(size=size, fill=Enrichment), shape=21)+
    ...
    endo_pathway
    ggsave(endo_pathway, file=paste("scratch/HLiCA/",names(sex_out_celltype)[celltype_num],"_sex_GSEA.png",sep=""), w=14,h=7)
  }
  
  endo_volcano<-makeVolcano(sex_out_celltype[[celltype_num]], 0.5, 0.05, paste("Differential expression between sexes\n",names(sex_out_celltype)[celltype_num], sep=""), round(max(abs(range(sex_out_celltype[[celltype_num]]$coef))),1)+0.2)
  endo_volcano
  ggsave(endo_volcano, file=paste("scratch/HLiCA/",names(sex_out_celltype)[celltype_num],"_sex_volcano.png",sep=""), w=10,h=7)
...
##########################################################################################################################
## Differential expression with AGE all endo
##########################################################################################################################
...
save_DEG_plots_age<-function(celltype_num){
  ...
  if(nrow(plt_path)==0){}else{
    ...
    endo_pathway
    ggsave(endo_pathway, file=paste("scratch/HLiCA/",names(age_out_celltype)[celltype_num],"_age_GSEA.png",sep=""), w=14,h=7)
  }
  
  endo_volcano<-makeVolcano_age(age_out_celltype[[celltype_num]], 0.5, 0.05, paste("Differential expression with age\n",names(age_out_celltype)[celltype_num], sep=""), round(max(abs(range(age_out_celltype[[celltype_num]]$coef))),1)+0.2)
  endo_volcano
  ggsave(endo_volcano, file=paste("scratch/HLiCA/",names(age_out_celltype)[celltype_num],"_age_volcano.png",sep=""), w=10,h=7)
