...
load(here("scratch/HLiCA/seu_cleaned_lymphocytes.RData"))
...
lymphocyte_volcano<-makeVolcano(out, 0.25, 0.05, "Differential expression between sexes", 0.6)
lymphocyte_volcano
ggsave(lymphocyte_volcano, file="scratch/HLiCA/lymphocyte_sex_volcano.png", w=10,h=7)


VlnPlot(seu_cleaned, features = c("KDM6A","HLA-A"), log = T, group.by = "donor_sex")
...
write.csv(plt_path_save, file="scratch/HLiCA/age_DEG_GSEA/lymphocyte_sex_GSEA.csv", row.names = FALSE)
...
lymphocyte_pathway<-ggplot(plt_path, aes(NES, reorder(pathway, NES)))+geom_point(aes(size=size, fill=Enrichment), shape=21)+
...
lymphocyte_pathway
ggsave(lymphocyte_pathway, file="scratch/HLiCA/lymphocyte_sex_GSEA.png", w=14,h=7)
...
head(sex_out_celltype[["Regulatory T Cell"]])



lymphocyte_volcano<-makeVolcano(sex_out_celltype[["Regulatory T Cell"]], 0.5, 0.05, "Differential expression between sexes", round(abs(max(range(sex_out_celltype[["Regulatory T Cell"]]$coef))),1))
lymphocyte_volcano
#ggsave(lymphocyte_volcano, file="scratch/HLiCA/lymphocyte_sex_volcano.png", w=10,h=7)
...
save_DEG_plots<-function(celltype_num){
  ...
  if(nrow(plt_path)==0){}else{
    ...
    lymphocyte_pathway
    ggsave(lymphocyte_pathway, file=paste("scratch/HLiCA/",names(sex_out_celltype)[celltype_num],"_sex_GSEA.png",sep=""), w=14,h=7)
  }
  
  lymphocyte_volcano<-makeVolcano(sex_out_celltype[[celltype_num]], 0.5, 0.05, paste("Differential expression between sexes\n",names(sex_out_celltype)[celltype_num], sep=""), round(max(abs(range(sex_out_celltype[[celltype_num]]$coef))),1)+0.2)
  lymphocyte_volcano
  ggsave(lymphocyte_volcano, file=paste("scratch/HLiCA/",names(sex_out_celltype)[celltype_num],"_sex_volcano.png",sep=""), w=10,h=7)
...
##########################################################################################################################
## Differential expression with AGE all lymphocytes
##########################################################################################################################
...
head(age_out_celltype[["Exhausted CD8 T Cell"]])




lymphocyte_volcano<-makeVolcano_age(age_out_celltype[["Exhausted CD8 T Cell"]], 0.5, 0.05, "Differential expression between sexes", 
                                    round(abs(max(range(age_out_celltype[["Exhausted CD8 T Cell"]]$coef))),1))
lymphocyte_volcano
#ggsave(lymphocyte_volcano, file="scratch/HLiCA/lymphocyte_sex_volcano.png", w=10,h=7)
...
save_DEG_plots_age<-function(celltype_num){
  ...
  if(nrow(plt_path)==0){}else{
    ...
    lymphocyte_pathway
    ggsave(lymphocyte_pathway, file=paste("scratch/HLiCA/",names(age_out_celltype)[celltype_num],"_age_GSEA.png",sep=""), w=14,h=7)
  }
  
  lymphocyte_volcano<-makeVolcano_age(age_out_celltype[[celltype_num]], 0.5, 0.05, paste("Differential expression with age\n",names(age_out_celltype)[celltype_num], sep=""), round(max(abs(range(age_out_celltype[[celltype_num]]$coef))),1)+0.2)
  lymphocyte_volcano
  ggsave(lymphocyte_volcano, file=paste("scratch/HLiCA/",names(age_out_celltype)[celltype_num],"_age_volcano.png",sep=""), w=10,h=7)
...
ggsave("scratch/HLiCA/CCL4_lymphocyte.png", w=10, h=5)
