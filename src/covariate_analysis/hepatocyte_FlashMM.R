...
load(here("scratch/HLiCA/seu_cleaned_hepatocytes.RData"))
...
save_DEG_plots<-function(celltype_num){
  ...
  if(nrow(plt_path)==0){}else{
    ...
    hepatocyte_pathway<-ggplot(plt_path, aes(NES, reorder(pathway, NES)))+geom_point(aes(size=size, fill=Enrichment), shape=21)+
    ...
    hepatocyte_pathway
    ggsave(hepatocyte_pathway, file=paste("scratch/HLiCA/",names(sex_out_celltype)[celltype_num],"_sex_GSEA.png",sep=""), w=14,h=7)
  }
  
  hepatocyte_volcano<-makeVolcano(sex_out_celltype[[celltype_num]], 0.5, 0.05, paste("Differential expression between sexes\n",names(sex_out_celltype)[celltype_num], sep=""), round(max(abs(range(sex_out_celltype[[celltype_num]]$coef))),1)+0.2)
  hepatocyte_volcano
  ggsave(hepatocyte_volcano, file=paste("scratch/HLiCA/",names(sex_out_celltype)[celltype_num],"_sex_volcano.png",sep=""), w=10,h=7)
...
##########################################################################################################################
## Differential expression with AGE all hepatocytes
##########################################################################################################################
...
save_DEG_plots_age<-function(celltype_num){
  ...
  if(nrow(plt_path)==0){}else{
    ...
    hepatocyte_pathway
    ggsave(hepatocyte_pathway, file=paste("scratch/HLiCA/",names(age_out_celltype)[celltype_num],"_age_GSEA.png",sep=""), w=14,h=7)
  }
  
  hepatocyte_volcano<-makeVolcano_age(age_out_celltype[[celltype_num]], 0.5, 0.05, paste("Differential expression with age\n",names(age_out_celltype)[celltype_num], sep=""), round(max(abs(range(age_out_celltype[[celltype_num]]$coef))),1)+0.2)
  hepatocyte_volcano
  ggsave(hepatocyte_volcano, file=paste("scratch/HLiCA/",names(age_out_celltype)[celltype_num],"_age_volcano.png",sep=""), w=10,h=7)
...
sig_genes_relaxed[grep("CYP",sig_genes_relaxed$gene),]

sig_genes_relaxed[which(sig_genes_relaxed$gene%in%c("CYP3A4", "CYP2A6", "MT-CO1", "CYP2C9", "CYP3A5","MT-CO2","HAL")),]


### smooth line violin plot
data_plot<-FetchData(seu_cleaned_age, vars = c("CYP3A4", "CYP2A6", "MT-CO1", "CYP2C9", "CYP3A5","MT-CO2","HAL"))
...
age_hepatocyte<-ggplot() +
  ...
  geom_text(data = sig_genes_relaxed[which(sig_genes_relaxed$gene%in%c("CYP3A4", "CYP2A6", "MT-CO1", "CYP2C9", "CYP3A5","MT-CO2","HAL")),],
            aes(x = 2, label = round(coef,2), y = ceiling(max(data_plot$value))), size=3)+
  ylim(0, ceiling(max(data_plot$value))*1.1)
age_hepatocyte

ggsave(age_hepatocyte, file="scratch/HLiCA/age_hepatocyte_genes.png", h=8, w=8)
