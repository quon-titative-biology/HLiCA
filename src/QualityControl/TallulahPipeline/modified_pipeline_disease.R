...
# Seurat version: 
# Hongru: SeuratObject_4.1.3, Seurat_4.3.0, and harmony 0.1.1.
# Rita and Jordan have the same version. 

###### Load Requirements ######
a1 = suppressPackageStartupMessages(require(methods))
a2 = suppressPackageStartupMessages(require(Matrix))          #   	- r-matrix=1.6_4
a3 = suppressPackageStartupMessages(require("DropletUtils"))
a4 = suppressPackageStartupMessages(require(dplyr))           #   	- r-dplyr=1.1.4
a5 = suppressPackageStartupMessages(require(Seurat))		  #   	- r-seurat=4.4.0
a6 = suppressPackageStartupMessages(require(optparse))
a7 = suppressPackageStartupMessages(require("SoupX"))
a8 = suppressPackageStartupMessages(require("ggplot2"))       #		- r-ggplot2=3.4.4
if (! (a1 & a2 & a3 & a4 & a5 & a6 & a7 & a8) ) {
	print("Cannot load all required packages: methods, Matrix, DropletUtils, dplyr, Seurat, optparse, SoupX", "ggplot2")
	exit()
}

script_dir = "/share/quonlab/workspaces/hrhu/HLiCA/diseased/TallulahPipeline/LiverMap2.0-master"
...
do_annotation <- function(myseur) {
        myseur <- run_scmap_seurat(myseur, scmap_ref=map1_ref);
				...
				general_labs2 <- simplify_annotations(myseur@meta.data$scmap_cluster_anno[[1]]) # correct to [[1]]
				...
				inconsistent <- myseur@meta.data$scmap_cell_anno != myseur@meta.data$scmap_cluster_anno[[1]]; # Correct to [[1]]
...
# OPTS$out_prefix = 'alignment/ref_GRCh38p13_gencode_v42/share_2023_05_13/share_qc/share_qc/Toronto/liver_006_NST_0_1_HY2YGCCXYSRR16227559/raw_feature_bc_matrix/qc_output/Cleaned_output'
...
library(data.table)
file_df = fread("/share/quonlab/HLiCA/czi_liver_disease_atlas/data/counts_BAM/file_df.csv", header=TRUE)

file.names = file_df$file_path
# Don't filter out gene to make sure all datasets have same set of genes
OPTS$cells = 0

for (file.name in file.names) {
	print(file.name)
	file.name = gsub(".h5", "", file.name) # # REMOVE THE .h5 from each file name
	...
	qc_not_finished = TRUE
		if (qc_not_finished) {
			...
			stats.to.save[['# of UMI']] = sum(n_umi_sorted)
			...
			# modified to set maximum indexing
			if (is_empty_threshold > n_umi_sorted[min(OPTS$min_cells*10,length(n_umi_sorted))]){
			...
			run_seurat_pipeline <- function(myseur, out_tag, do_annot=T) {
				# Normalize
			...
			if (status$emptydrops == 'Complete') {
				status <- tryCatch(
					...
					anno_cluster <- anno_cluster[,1:2] # Added in because code requires anno_cluster$lab
					colnames(anno_cluster) = c('cluster','lab')
					...
					error=function(e){
						status = data.frame(soupx='Incomplete')
						return(status)
					}
				)
				write.table(status,csv.status.file,sep=',')
