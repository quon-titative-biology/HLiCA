...
auto_anno_dir <- "LiverMap2.0-master"

#map1_ref <- readRDS(paste(auto_anno_dir,"scmap_reference.rds", sep="/"))
map1_ref <- readRDS(paste(auto_anno_dir,"Map1_scmap_minimal_reference.rds", sep="/"))
...
my_markers <- function(mat) {
        ...
        my_split_max_gap <- function(x) {
                x <- sort(x)
                jumps <- diff(x);
                br_pt <- which(jumps == max(jumps))[[1]] # fails in case of multiple matches
                return(c(x[br_pt], max(jumps)));
        }
        thresh <- apply(mat, 1, my_split_max_gap); # should be dim of n x 2
...
#	require("SingleR")
#	out <- SingleR(method="single", myseur@assays$RNA@data, logcounts(ref), map1_ref$cell_type1)
#}
